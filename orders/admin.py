...
class OrderAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'org', 'date', 'due_date', 'status', 'worknotes_links', 'docket_link', 'shipping_links', 'invnum_form')
    search_fields = ('name', 'invoice_number')
    raw_id_fields = ('ship_to',)
    date_hierarchy = ('date')
    fieldsets = (
        ('Order info', { 'fields': ('name', 'status', 'placed_by', 'org', 'date', 'due_date', 'po_number', 'additional_info', 'approved_by', 'approved_date', 'invoice_number', 'ship_to')
        }),
    )
