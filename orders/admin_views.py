...
from mimicprint.orders.models import Cart, Order, InventoryHistory, OrderedItem, WorkNote
from mimicprint.addresses.models import Address
from mimicprint.products.models import Product
from mimicprint.orders.forms import FastOrderForm
from mimicprint.orders.views import subtract_inventory
import datetime
...
def products_ordered(request, order_id):
    """
    Shows all the products on an order. If the ordered item is variable, the template displays a link so that an admin can download a printready PDF.
    """
...
def save_invnum(request):
    ...
    Note that we do *no* checking on the invoice number: whether it matches a certain pattern, whether it is unique, etc. We trust the user (and it can be edited afterwards if necessary).
    """
