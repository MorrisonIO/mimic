...
class Cart:
    ...
    def add_item(self, product, unique_id, quantity):
        """
        Adds an Item to a Cart. This method is also used when modifying an Item already in a Cart; in that case, the existing quantity is overwritten (instead of adding another of the same product).
        """
...
class Order(models.Model):
    """
    An Order is created when the user completes the 'checkout' procedure: adding products to a Cart, choosing a shipto recipient, etc and submitting the form.
    """
    STATUS_CHOICES = (
    ...
    def make_name(self):
        ...
        chars = random.sample(['B','C','D','F','G','H','J','K','L','M','N','P','Q','R','S','T','V','W','X','Z'], 4) # 4 random consonants. No vowels so we don't get any, uh, 4 letter words. :-p
    ...
    def get_status_explanation(self):
        ...
        """
        if self.status == 'pa':
            return "There is a product on the order that has been flagged as requiring approval from a manager before it can be placed into production. Please note that we take no action on all items on this order until approval has been received. When this order is approved, its status will change to Active."
        elif self.status == 'ac':
    ...
    def worknotes_links(self):
        ...
        try:
            ...
            return '<a href="/admin/orders/worknote_view/%s/" title="%s">View</a> / <a href="/admin/orders/worknote/%s/">Edit</a>' % (worknote.id, worknote.status, worknote.id)
        except: 
    ...
    def shipping_links(self):
        ...
        """
        return '<a href="/admin/orders/shipping/packing_slip/%s/">PS</a> | <a href="/admin/orders/shipping/label/%s/">L</a> | <a href="/admin/orders/shipping/comm_inv/%s/">CI</a>' % (self.id, self.id, self.id)
    ...
    def invnum_form(self):
        ...
        if self.invoice_number:
            ...
            return '<form action="/admin/orders/save_invnum/" method="post"><input type="text" size="10" name="invoice_number"><input type="hidden" name="order_id" value="%s"> <input type="submit" value="&gt;"></form>' % self.id
...
class InventoryHistory(models.Model):
    """
    An inventory history is an event where a product's inventory changes: typically this is either an order, or a manual adjustment (eg after a recount, or when new products are created). 

    Note that because OrderedItems reference this model, an InventoryHistory event is generated whenever an order is placed, *regardless* of whether we are tracking inventory on the product in question.
    """
    product = models.ForeignKey(Product)
    order = models.ForeignKey(Order, blank=True, null=True, help_text="<strong>Leave this blank</strong> if you are making a manual adjustment to inventory levels.")
    amount = models.IntegerField(max_length=15, help_text="Enter the amount the inventory is changing by, <em>not</em> the new inventory level. You may enter negative numbers. For example, if you are adding 100 pieces of stock, enter 100; if physical inventory is 25 less than what it should be, enter -25.")
...
class OrderedItem(models.Model):
    """ 
    An ordered item is one product on an order. An order is made up of one or more ordered items.
    """
...
class WorkNote(models.Model):
    """
    Used to keep track of current progress of work on an order -- who's done what, current status, etc -- basically a freeform area to jot notes in.
    """
    order = models.ForeignKey(Order, unique=True)
    staff = models.ManyToManyField(User, help_text="Select the Mimic employees involved in this order.", verbose_name="Mimic staff", limit_choices_to = {'is_staff__exact': '1'})
    status = models.CharField(max_length=200, help_text="Enter a short summary of the progress of this order.")
    notes = models.TextField(help_text="<p class='help'>Enter any workflow related notes here. Formatting hints:</p><p class='help'>Make text <tt>*italic* or __bold__</tt>; full links become clickable<br>End a line with 2 or more spaces for a carriage return, double-space for paragraphs<br>Make a line with <tt>---</tt>, a header with <tt>### Something</tt><br>Make a bullet list with <tt>* item</tt> on separate lines, a numbered list with <tt>1. item</tt><br>To colour-code your text, surround it with your uppercase initials like this: <tt>DC David wrote this /DC</tt></p>", blank=True)
